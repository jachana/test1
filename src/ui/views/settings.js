...
export function settingsView({ rerender }) {
  const automation = card('⚙️ Automation', [
    ...
    toggle('Auto-drink potions', () => S.settings.autoPotion, (v) => { S.settings.autoPotion = v; },
      'Drinks the cheapest potion that covers the wound — the strongest you have when nothing does. One every two seconds.'),
    (() => {
    ...
    toggle('Walk back after dying', () => S.settings.autoReturn, (v) => { S.settings.autoReturn = v; },
      'Returns to the same hunting ground after a death, so offline runs keep going. Three deaths inside ten minutes stops it anyway.'),
  ...
  const sound = card('🔊 Sound', [
    el('p', { class: 'muted small', text: 'Short synthesised blips — swings, kills, levels and rare drops. Off by default, because an idle game lives in a background tab.' }),
    toggle('Sound effects', () => S.settings.sound, (v) => {
      S.settings.sound = v;
      setSound(v, S.settings.volume);
      if (v) play('level'); // so you know it worked and how loud it is
    }, 'Nothing is loaded or created until you switch this on.'),
    (() => {
      const slider = el('input', { type: 'range', min: '0', max: '100', step: '5', value: String(Math.round(S.settings.volume * 100)) });
      const out = el('span', { class: 'tag', text: `${Math.round(S.settings.volume * 100)}%` });
      slider.addEventListener('input', () => {
        S.settings.volume = Number(slider.value) / 100;
        out.textContent = `${slider.value}%`;
        setSound(S.settings.sound, S.settings.volume);
      });
      slider.addEventListener('change', () => { if (S.settings.sound) play('kill'); });
      return el('label', { class: 'field' }, ['Volume', el('div', { class: 'row' }, [slider, out])]);
    })(),
  ]);
